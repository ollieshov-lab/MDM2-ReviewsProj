...
from rp_paths import resolve_existing_path, resolve_output_file

# ── CONFIG ────────────────────────────────────────────────────────────────────
MODEL_ROOT = os.fspath(
    resolve_existing_path(
        "outputs/models/zeroshot_tags",
        "results_zeroshot_tags",
        "results",
        env_var="RP_TAG_MODEL_DIR",
    )
)

MODEL_PATHS = {
    "Couple":                     os.path.join(MODEL_ROOT, "Couple_bertopic_model"),
    "Family with young children": os.path.join(MODEL_ROOT, "Family_with_young_children_bertopic_model"),
    "Group":                      os.path.join(MODEL_ROOT, "Group_bertopic_model"),
    "Solo traveler":              os.path.join(MODEL_ROOT, "Solo_traveler_bertopic_model"),
}

CSV_PATHS = {
    "Couple":                     os.path.join(MODEL_ROOT, "Couple_topics.csv"),
    "Family with young children": os.path.join(MODEL_ROOT, "Family_with_young_children_topics.csv"),
    "Group":                      os.path.join(MODEL_ROOT, "Group_topics.csv"),
    "Solo traveler":              os.path.join(MODEL_ROOT, "Solo_traveler_topics.csv"),
}

OUTPUT_FILE = os.fspath(
    resolve_output_file(
        "RP_TOPIC_HIERARCHY_OUTPUT_FILE",
        "outputs/interactive/bertopic_hierarchy.json",
    )
)
# ── END CONFIG ────────────────────────────────────────────────────────────────
