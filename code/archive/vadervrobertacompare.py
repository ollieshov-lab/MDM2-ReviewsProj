...
TABLE_DIR = os.fspath(
    resolve_output_dir(
        "RP_SENTIMENT_COMPARISON_OUTPUT_DIR",
        "outputs/tables/sentiment_model_comparison",
    )
)
PLOT_DIR = os.fspath(
    resolve_output_dir(
        "RP_SENTIMENT_COMPARISON_PLOT_DIR",
        "outputs/plots/sentiment_model_comparison",
    )
)
CACHE_PATH = os.path.join(TABLE_DIR, "scored_sample.csv")
...
plt.tight_layout()
bar_path = os.path.join(PLOT_DIR, "per_class_accuracy.png")
...
sample_df.to_csv(os.path.join(TABLE_DIR, "scored_sample_with_labels.csv"), index=False)
cm_roberta.to_csv(os.path.join(TABLE_DIR, "roberta_confusion_matrix.csv"))
cm_vader.to_csv(os.path.join(TABLE_DIR, "vader_confusion_matrix.csv"))
acc_df.to_csv(os.path.join(TABLE_DIR, "per_class_accuracy.csv"), index=False)
with open(os.path.join(TABLE_DIR, "classification_reports.txt"), "w", encoding="utf-8") as handle:
    handle.write("RoBERTa Classification Report\n")
    handle.write(classification_report(true_labels, roberta_labels, target_names=LABEL_ORDER))
    handle.write("\nVADER Classification Report\n")
    handle.write(classification_report(true_labels, vader_labels, target_names=LABEL_ORDER))
