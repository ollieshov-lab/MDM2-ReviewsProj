...
# ── Output Dir ────────────────────────────────────────────────────────────────
os.makedirs('results_tags', exist_ok=True)
os.makedirs('results_tags/embeddings', exist_ok=True)
...
for traveler_type in traveler_types:
    ...
    # ── Embed ─────────────────────────────────────────────────────────────────
    EMBEDDING_PATH = f"results_tags/embeddings/{safe_name}_embeddings.npy"
...
# ── Sentiment (stratified sample of 36,000 fragments) ────────────────────
    ...
    SENTIMENT_PATH = f"results_tags/{safe_name}_Sentiment_Scores.csv"
    ...
    # ── Save Document Info ────────────────────────────────────────────────────
    doc_inf.to_csv(f"results_tags/{safe_name}_Document_Info.csv", index=False)
...
# ── Topic Sentiment Aggregation (scored fragments only) ──────────────────
    ...
    hotel_topic_sentiment.to_csv(
        f"results_tags/{safe_name}_Topic_Sentiment.csv", index=False
    )
    ...
    # ── Save Model ────────────────────────────────────────────────────────────
    model_path = f"results_tags/{safe_name}_bertopic_model"
    model.save(
        model_path,
...
print("\n✓ All done — outputs in ./results_tags/")