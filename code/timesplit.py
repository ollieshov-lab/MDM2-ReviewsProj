...
for season in ['Winter', 'Spring', 'Summer', 'Autumn']:
    ...
    # save docs WITH their assigned topic
    docs = season_dfs[season][REVIEW_COL].dropna().tolist()
    docs = [d for d in docs if d.strip()]
    topics, _ = model.transform(docs)
    docs_df = pd.DataFrame({'text': docs, 'topic': topics})
    docs_df.to_csv(f'results/{season}_docs.csv', index=False)
    print(f"Saved results/{season}_docs.csv")
