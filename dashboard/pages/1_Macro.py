...
CITY_BENCH = DATA_DIR / "Hotel_City_Topic_Benchmark.csv"
HTL_BENCH  = DATA_DIR / "Hotel_Hotel_vs_City_Benchmark.csv"
CORR_FILE  = DATA_DIR / "Hotel_Topic_ReviewerScore_Correlations.csv"
DOC_FILE   = DATA_DIR / "Hotel_Document_Info.csv"
SENT_FILE  = DATA_DIR / "Hotel_Sentiment_Scores.csv"
SAMP_FILE  = resolve_repo_path("Datasets/Hotel_Reviews_StratSamp_Balanced.csv")
...
def load_data():
    ...
    if os.path.exists(DOC_FILE) and os.path.exists(SENT_FILE):
        ...
        if os.path.exists(SAMP_FILE):
            ...
            # Regenerate deterministically to mirror the main hotel pipeline.
            df = raw.copy()
...
if not os.path.exists(RAW_CSV):
    st.error("Hotel_Reviews.csv not found at Datasets/Hotel_Reviews.csv. Cannot run the macro dashboard.")
    st.stop()
...
with tab6:
    ...
    if frag_df.empty:
        st.warning(
            f"BERTopic fragment data not found in {HOTEL_PIPELINE_TABLES_REL}. "
            ...
            if st.button("Run hypothesis test", type="primary"):
                ...
                if len(a_scores) == 0:
                    ...
                    if len(a_scores) < MIN_N:
                        st.warning(f"{group_a} only has {len(a_scores)} fragments for this topic — interpret with caution.")
                    if len(b_scores) < MIN_N:
                        st.warning(f"{group_b} only has {len(b_scores)} fragments for this topic — interpret with caution.")
